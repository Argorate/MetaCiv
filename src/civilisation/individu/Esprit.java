...
public class Esprit {
	...
	/**
	 * @param t
	 * @return first cogniton of type t (or null if there is any cogniton of this type)
	 */
	...
	/**
	 * @param t, type of cigniton
	 * @return ArrayList of all cognitons of type t
	 */
	public ArrayList<CCogniton> getCognitonsOfType(TypeDeCogniton t) {
		ArrayList<CCogniton> list = new ArrayList<CCogniton>();
		for(int i = 0 ; i < cognitons.size() ; i++)
		{
			if(cognitons.get(i).getCogniton().getType().equals(t))
				list.add(cognitons.get(i));
		}
		return list;
	}
	
	
	public CCogniton getCogniton(String name)
	{
		int i = 0;
		while(cognitons.get(i).cogniton.getNom() != name)
		{
			i++;
		}
