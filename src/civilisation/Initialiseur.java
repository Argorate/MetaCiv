...
public class Initialiseur {
	...
	public Initialiseur(){
		...
		Configuration.objets = new ArrayList<Item>();
		...
		for (File file : files) {
		    ...
		    if (file.isFile()) {
		       ...
		       //	System.out.println("array : "+ plans);
	       		...
	       		/*ajoutes les data concernant les Need cogniton */
		       	liste = Initialiseur.getListeChamp("NeedValue", file);	      
		       	for (int i = 0 ; i < liste.size(); i++){
		       		((Need) listeCognitons.get(nom)).setValue(Integer.parseInt(liste.get(i)[0]));
		       	}
		       	
		       	liste = Initialiseur.getListeChamp("NeedDecrease", file);	      
		       	for (int i = 0 ; i < liste.size(); i++){
		       		((Need) listeCognitons.get(nom)).setDecrease(Integer.parseInt(liste.get(i)[0]));
		       	}
		       	
		       	liste = Initialiseur.getListeChamp("NeedSatisfactions", file);	      
		       	for (int i = 0 ; i < liste.size(); i++){
		       		((Need) listeCognitons.get(nom)).satisfactions.put(liste.get(i)[0], Integer.parseInt(liste.get(i)[1]));
		       	}
		    }
		}	
		}
